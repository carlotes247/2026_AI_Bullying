...
public class CrawlerAgent_Pablo : Agent
{
    ...
    [Header("Recovery")]
    [Space(10)]
    [Tooltip("Minimum body height considered fallen")]
    public float minBodyHeight = -1.5f;
    [Tooltip("Body height at which standing is considered stable")]
    public float targetBodyHeight = -1f;
    [Tooltip("Penalty factor for angular velocity (multiplied by squared magnitude)")]
    public float angularVelocityPenalty = 0.0005f;

    [Header("Debug_Agent")]
    public Vector3 m_BodyUp;
    public float m_DotProductBodyUp;
    public float m_Upright;
    public float m_Height;
    public float m_Stability;
    public float m_MoveReward;
    public float m_RootAngVelSq;
    public float m_RootAngleVelSqReward;
    ...
    public override void Initialize()
    {
        ...
        // Cache root body part (the transform `body`) for easier access to its Rigidbody
        if (m_JdController.bodyPartsDict != null && m_JdController.bodyPartsDict.ContainsKey(body))
        {
            m_rootBodyPart = m_JdController.bodyPartsDict[body];
        }
    }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        body.rotation = Quaternion.Euler(flipped ? 180f : 0f, yaw, 0f);
    ...
    void FixedUpdate()
    {
        ...
        // Reward 1 stability = upright * height
        // upright
        m_BodyUp = body.up;
        m_DotProductBodyUp = Vector3.Dot(body.up, Vector3.up);
        float upright = Mathf.Clamp01((m_DotProductBodyUp + 1f) * 0.5f);
        m_Upright = upright;
        float height = Mathf.InverseLerp(minBodyHeight, targetBodyHeight, body.position.y);
        m_Height = height;
        float stability = upright * height;
        m_Stability = stability;

        var moveReward = matchSpeedReward * lookAtTargetReward;
        m_MoveReward = moveReward;

        AddReward(0.1f * stability);
        AddReward(0.9f * moveReward * Mathf.Lerp(0.25f, 1f, stability));

        // Root Angle Vel Sq seems to be penali
        float rootAngVelSq = 0f;
        if (m_rootBodyPart != null && m_rootBodyPart.rb != null)
        {
            rootAngVelSq = m_rootBodyPart.rb.angularVelocity.sqrMagnitude;
        }
        m_RootAngVelSq = rootAngVelSq;
        m_RootAngleVelSqReward = -angularVelocityPenalty * rootAngVelSq;
        AddReward(-angularVelocityPenalty * rootAngVelSq);
    }
